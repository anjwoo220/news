...
def analyze_hotel_reviews(hotel_name, rating, reviews, api_key):
    ...
    try:
        ...
        **[작성 가이드라인 - 엄격 준수]**
        1. **추측 금지:** "확인이 필요함", "있을 수도 있음", "불명확함"과 같은 불확실한 추측성 내용은 단점에 절대 적지 마.
        2. **비판적 시각 (확실한 불만):** 리뷰나 데이터에서 **명확하게 지적된 부정적 요소**만 포함해. (예: "방음이 안 됨", "수압이 약함", "조식 종류가 적음")
        3. **한국인 맞춤:** "김치가 있는지 확인 필요" 같은 정보 부재는 언급하지 마. 오직 "입맛에 안 맞음", "먹을 게 없음"이라고 **직접 불평한 경우에만** 작성해.
        4. **정보 부재 시 생략:** 확실한 단점이 없으면 억지로 만들어내지 마.
