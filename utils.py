...
def generate_tour_itinerary(tours, region="방콕"):
    ...
    try:
        ...
        [필수 고려사항]
        1. **투어 시간 및 기간 (매우 중요)**:
           - 상품 태그에 **'전일투어'** 또는 **'종일'**이 포함된 투어는 하루 전체(8~10시간)를 소요하는 것으로 간주하여 그날은 다른 큰 일정을 잡지 마세요.
           - **'반일투어'**는 약 4~5시간 소요되므로 오전 또는 오후 중 하나에 배치하고, 남는 시간에는 가벼운 자유 일정이나 다른 반일/짧은 투어를 배치하세요.
        2. **계절 및 날씨 (중요)**: 
